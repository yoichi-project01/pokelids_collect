...
export default function Home() {
  ...
  // 🌟 追加：現在選択されている都道府県（初期値は "all" で全国表示）
  ...
  // 🌟 変更：都道府県フィルターと状態フィルターの両方を組み合わせて絞り込む
  const filteredPokelids = mergedPokelids.filter((lid) => {
    ...
    // 2. 都道府県の絞り込み（IDを1000で割った整数部分が都道府県コードになります）
    let prefMatch = true;
      ...
      <div className="absolute top-0 left-0 w-full z-20 pointer-events-none">
        ...
        <div className="w-full overflow-x-auto whitespace-nowrap px-4 py-3 mt-1 pointer-events-auto no-scrollbar flex items-center gap-2">
          ...
          <select
            ...
            className="px-3 py-2 rounded-full text-sm font-bold shadow border border-gray-200 bg-white text-gray-700 outline-none cursor-pointer hover:bg-gray-50"
          >
