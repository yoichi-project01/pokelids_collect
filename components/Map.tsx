...
import imageCompression from "browser-image-compression"; // 画像サイズを小さくするツール
import exifr from "exifr"; // 写真の裏側（GPS情報など）を解析するツール
...
export default function Map({ pokelids, onCheckIn }: MapProps) {
  ...
  const [selectedLid, setSelectedLid] = useState<MapProps["pokelids"][0] | null>(null); // クリックされたポケふた
  const [currentPosition, setCurrentPosition] = useState<{ lat: number; lng: number } | null>(null); // 現在地
  const [isUploading, setIsUploading] = useState(false); // 画像の送信中フラグ

  const onLoad = useCallback((map: google.maps.Map) => { mapRef.current = map; }, []);
  const onUnmount = useCallback(() => { mapRef.current = null; }, []);

  // 自分の現在地を取得して、地図をそこへ移動させる処理
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, lidId: number) => {
    ...
    try {
      // EXIF情報を解析する
      let checkinStatus = "silver"; // デフォルトは「銀メダル（写真チェックイン）」
      try {
        const gpsData = await exifr.gps(file);
        if (gpsData && gpsData.latitude && gpsData.longitude) {
          // 選んでいるポケふたの「本来の座標」を取得
          const targetLid = pokelids.find(l => l.id === lidId); 
          if (targetLid) {
            // 写真のGPSと、ポケふたのGPSの距離を計算
      ...
      setSelectedLid(null); // ポップアップを閉じる
    } catch (err) {
        ...
        {pokelids.map((lid) => {
          ...
          return (
            <Marker 
              ...
              // 訪問済みなら自分の写真、未訪問なら公式画像をピンにする
              icon={{ url: lid.user_image_url || lid.image_url || "https://placehold.co/100?text=No+Img", scaledSize: new window.google.maps.Size(50, 50) }} 
              opacity={lid.visited ? 1.0 : 0.6} // 未訪問は少し半透明にする
            />
          );
        })}

        {/* 吹き出し（ピンをクリックした時に出るポップアップ） */}
        {selectedLid && (
